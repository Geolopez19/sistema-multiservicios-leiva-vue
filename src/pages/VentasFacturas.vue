<template>
  <div class="max-w-7xl mx-auto">
    ...
    <div class="bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl shadow-xl p-8 mb-6">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-green-100 overflow-hidden">
      ...
      <Column field="invoice_number" header="#">
        <template #body="{ data }">{{ data.invoice_number ?? data.number ?? '—' }}</template>
      </Column>
      ...
      <Column header="Acciones">
        <template #body="{ data }">
          <div class="flex gap-2">
            <Button type="button" icon="pi pi-eye" severity="secondary" text rounded @click="(e) => { e.preventDefault(); e.stopPropagation(); openOrder(data); }" v-tooltip.top="'Ver detalles'" />
            <Button type="button" icon="pi pi-print" severity="secondary" text rounded @click="(e) => { e.preventDefault(); e.stopPropagation(); handlePrint(data, e); }" v-tooltip.top="'Imprimir'" />
    ...
    <!-- Drawer de Detalle -->
    <Drawer v-model:visible="drawerVisible" position="right" class="w-full md:w-[600px] lg:w-[800px] modern-drawer">
      <template #header>
        <div class="flex items-center gap-3">
          <span class="text-xl font-bold text-white">Factura #{{ currentOrder?.invoice_number ?? currentOrder?.number ?? '—' }}</span>
      ...
      <div class="flex flex-col h-full bg-gradient-to-b from-slate-50 to-white">
        <div class="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <Panel header="Información del Cliente">
            ...
            <div v-if="currentOrder?.notes" class="mt-4 pt-4 border-t border-gray-100">
              <span class="text-sm font-semibold text-gray-500 uppercase block mb-1">Notas</span>
              <p class="text-gray-700">{{ currentOrder.notes }}</p>
            </div>
          </Panel>

          <!-- Productos -->
          <Panel header="Productos">
            <DataTable :value="items" class="p-datatable-sm" :loading="loadingItems">
              ...
              <Column field="line_total" header="Total" class="text-right font-semibold">
          ...
          <div class="flex justify-end">
            <div class="w-full md:w-64 space-y-2 bg-gray-50 p-4 rounded-lg border border-gray-200">
              ...
              <div class="flex justify-between text-sm">
                ...
                <span>{{ formatCurrency(currentOrder?.tax_total || 0) }}</span>
              </div>
              <div class="flex justify-between text-xl font-bold border-t border-gray-200 pt-2">
                <span>Total:</span>
                <span class="text-indigo-600">{{ formatCurrency(currentOrder?.total || 0) }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Footer del Drawer -->
        <div class="p-6 border-t border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 flex justify-end gap-2">
          <Button 
            type="button"
          ...
          />
          <Button type="button" label="Imprimir" icon="pi pi-print" severity="secondary" @click="(e) => { e.preventDefault(); handlePrint(currentOrder, e); }" />
          <Button type="button" label="Descargar PDF" icon="pi pi-download" severity="success" @click="(e) => { e.preventDefault(); handleDownloadPDF(currentOrder, e); }" />
        </div>
      </div>
    </Drawer>
  </div>
...
const filteredOrders = computed(() => {
  ...
  const filtered = orders.value.filter(o => o.status === 'paid' || o.status === 'cancelled')
...
const confirmCancelOrder = async () => {
  if (!confirm('¿Estás seguro de cancelar esta venta? El stock se repondrá automáticamente.')) return
